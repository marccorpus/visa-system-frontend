...
router.beforeEach(async (to, from, next) => {
  ...
  if (hasToken) {
    if (to.path === "/login" || to.path === "/reset-password") {
      ...
      // Open sidebar if ClientProfile/GroupProfile Page, else close
      const isOpen = store.getters.sidebar.opened;

      if (isOpen && (to.name == "ClientProfile" || to.name == "GroupProfile")) {
        store.dispatch("app/closeSideBar", { withoutAnimation: false });
      } else if (
        !isOpen &&
        to.name != "ClientProfile" &&
        to.name != "GroupProfile"
      ) {
